...
CREATE TABLE `sys_scheduled` (
  ...
  `enable` varchar(2) NOT NULL DEFAULT '0' COMMENT '是否启用定时任务（1-启用，0-不启用）',
  `open_log` varchar(2) DEFAULT '1' COMMENT '是否开启日志（1-开启，0-不开启）',
  `type` varchar(2) NOT NULL COMMENT '类型（ANNOTATION-注解，TEMPLATE-模版）',
